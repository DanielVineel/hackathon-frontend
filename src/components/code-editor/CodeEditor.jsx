...
const CodeEditor = ({
  ...
  useEffect(() => {
    if (testResults && testResults.length > 0) {
      console.log("📊 Test results received, switching to results tab", { count: testResults.length, passed: testResults.filter(r => r.passed).length });
  ...
  return (
    <div className={`code-editor-container mode-${mode}`}>
        ...
        {showButtons && (
          <div className="toolbar-right">
            <button
              ...
              onClick={() => {
                console.log("Run Tests button clicked", { loading, hasCode: !!code.trim() });
                if (onRunTests) {
                  onRunTests();
                } else {
                  console.error("onRunTests is not defined");
                }
              }}
      ...
      {/* Tab for results (if in event mode or have results) */}
      {(mode === "event" || testResults.length > 0) && (
        <div className="editor-tabs">
          ...
          {testResults.length > 0 && (
            ...
            >
              Test Results ({passedCount}/{testResults.length})
            </button>
          )}
        </div>
      )}
        ...
        <div className="test-results-panel">
          ...
          {testResults.length > 0 && (
            <div className="results-summary">
              ...
              <div className="summary-stats">
                ...
                <span className="stat total">Total: {testResults.length}</span>
              </div>
            </div>
          )}
          ...
          <div className="test-results-list">
            {testResults.map((result, idx) => {
              ...
              return (
                <div 
                  key={`result-${idx}-${result.input?.substring(0, 10) || 'hidden'}`} 
                  className={`result-card ${result.passed ? "passed" : "failed"}`}
                >
                  <div className="result-header">
                    <span className="result-num">
                      {isHidden ? "🔒 Hidden Test" : `Test ${idx + 1}`}
                    </span>
                    ...
                    {!isHidden ? (
                      <>
                        <div className="result-io">
                          ...
                          <pre className="result-value">{result.input || "(no input)"}</pre>
                        </div>
                        <div className="result-io">
                          <span className="label">Expected Output:</span>
                          <pre className="result-value">{result.expected || "(no expected output)"}</pre>
                        </div>
                        <div className="result-io">
                          <span className="label">Your Output:</span>
                          <pre className={`result-value ${result.passed ? "correct" : "wrong"}`}>
                      ...
                      <div className="hidden-test-info">
                        <p className="hidden-label">🔒 Hidden Test Case</p>
                        <p className="hidden-status">
                          Status: <strong className={result.passed ? "passed" : "failed"}>
                            {result.passed ? "✓ PASSED" : "✗ FAILED"}
                          </strong>
                        </p>
                        {result.error && (
                          <p className="hidden-error">Error: {result.error}</p>
                        )}
                      </div>
                    )}
